    ...
    Add Textbook/s
               ...
               ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a rel="tooltip" title="ListedBook" id="e<?php echo $book_name; ?>" href="listedbook.php<?php echo '?id=' . $book_name; ?>" class="btn btn-success"><i class="bx bxs-edit"></i>&nbsp;View</a>
                                        </td>
...
if (isset($_POST['save'])) {
    ...
    if (mysqli_num_rows($query1) > 0) {
        ...
        for ($i = 0; $i < $qty ; $i++)
        {
        echo ('<script>alert("Book/s Successfully Added ")</script>');
        mysqli_query($connection, "INSERT into books (name,date) values ('$bookname',NOW())") or die($connection->error);
        echo ('<script>location.href = "books.php"</script>');
        }
